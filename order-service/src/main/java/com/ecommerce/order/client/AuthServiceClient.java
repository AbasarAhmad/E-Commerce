    ...
    public UserResponse getUserByUsername(String username,String authorizationHeader) {
        try {
            return restClient
                    ...
                    .uri(
                        authServiceUrl
                            + "/api/v1/auth/internal/users/{username}",
                        username
                    )
            ...
            throw new AuthServiceException("Authentication failed while calling Auth Service",HttpStatus.UNAUTHORIZED,e);

        } catch (HttpClientErrorException.NotFound e) {
            throw new AuthServiceException("User not found in Auth Service",HttpStatus.NOT_FOUND,e);
        } catch (HttpClientErrorException e) {
            throw new AuthServiceException("Auth Service rejected the request: "+ e.getStatusCode(),HttpStatus.BAD_GATEWAY,e);

        } catch (HttpServerErrorException e) {
            throw new AuthServiceException("Auth Service returned a server error: "+ e.getStatusCode(),HttpStatus.BAD_GATEWAY,e);
