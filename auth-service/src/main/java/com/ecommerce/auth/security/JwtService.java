...
public class JwtService {
	...
	public String generateToken(String username, String role) {
		...
		String token = Jwts.builder().subject(username).claim("role", role).issuedAt(now).expiration(expiration)
	...
	public String extractUsername(String token) {
		...
		String username = Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(token).getPayload()
				.getSubject();
	...
	public String extractRole(String token) {
		...
		String role = Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(token).getPayload().get("role",
				String.class);
